...
- **Client:** the `minio` Python SDK's `MinioAdmin` (admin API over HTTP) + `Minio` (bucket ops) — consistent with how Keycloak uses `requests` (no `kubectl exec`; MinIO has a real admin API). Methods as they exist in the pinned `minio==7.2.15` (verified by introspecting the installed package): `user_add(access_key, secret_key)`, `policy_add(policy_name, policy_file)` — **`policy_file` is a filesystem path**, so the policy JSON is written to a temp file; `policy_set(policy_name, user=<name>)`; `user_list()` — **returns a JSON string** (parse with `json.loads`, then membership-test the access key); `Minio.make_bucket/bucket_exists`.
