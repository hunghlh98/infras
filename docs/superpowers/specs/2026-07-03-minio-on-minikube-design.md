...
| `minio/scripts/deploy.sh` | **Single deploy entrypoint** — operator + Vault-sourced creds + secrets + tenant + ingress (cf. postgres/scripts/deploy.sh) |
| `minio/operator/kustomization.yaml` | Operator overlay: pinned v7.1.1, retargeted to `infras-minio`, 1 replica |
| `minio/tenant.yaml` | `Tenant` CR: pool `minio-pool`, 1 server × 4 drives, EC:2, `requestAutoCert: false` |
| (secrets) | `storage-configuration`/`storage-user` — generated by `deploy.sh` from Vault, not a committed manifest |
...
### Credentials
- `deploy.sh` generates a random **20-char** root password (house style, `/dev/urandom | tr -dc 'A-Za-z0-9' | head -c 20`), stores it in **Vault** at `infras/minio/root` (source of truth; reused on re-runs), and creates the `storage-configuration` k8s Secret from it — **nothing hardcoded on disk**. This mirrors `postgres/scripts/deploy.sh` (`infras/postgres/auth`).
- The Operator's Tenant still consumes creds from the `storage-configuration` Secret (that's the only way it can) — but that Secret is *generated from Vault at deploy time*, not committed.
- **Phase 2 (follow-up):** inject the Tenant's Secret directly from Vault (Vault Agent / External Secrets Operator) so it never materializes as a plain k8s Secret; and use a dedicated MinIO admin user instead of root.
...
- **`create_acl`:** ensure bucket → `policy_add(app-<name>, <tempfile with scoped JSON>)` → `user_add(<name>, <password>)` → `policy_set(app-<name>, user=<name>)` → inherited `_store_credential` writes `minio.username`/`minio.password` to `infras/minio/<name>` and the app secret. Idempotent (bucket/user/policy upserts).
- **`verify_acl`:** `service_name in json.loads(admin.user_list())`. Infra errors (endpoint/Vault unreachable) propagate → API returns 502 "could not verify", never a misleading "not found" (matches the codebase's corrected behavior).
